...
- [ ] At startup, bot validates each managed market by reading `adapter.ids(marketParams)` and verifying every returned id has `absoluteCap > 0` on the vault — markets with no cap are logged as `"ignored: no absolute cap configured"` and excluded from the rebalance loop
- [ ] **At startup, bot refuses to start if any managed market has `relativeCap == 0` on any of its 3 ids.** This is almost certainly a misconfiguration (`relativeCap == 0` means "allocation must be 0", effectively forbidding the market — the intended "no relative cap" sentinel is `relativeCap == WAD`). The error message must be explicit, name the offending market label and the offending cap id, and tell the operator: `"Refusing to start: market <label> has relativeCap == 0 on cap id <id>. This forbids any allocation to this market. If you meant 'no relative cap', set relativeCap to WAD (1e18). If you meant to forbid this market, remove it from MANAGED_MARKETS."`
...
| Default `relativeCap` value the deploy script writes for new managed markets? | **`WAD` (`1e18`)** — meaning "no relative cap; rely on absoluteCap as the hard ceiling". The script lets the operator override per market via the JSON config, but defaults to WAD so that a fresh deploy never accidentally writes a `relativeCap == 0` (which would forbid the market). | Decided 2026-04-07 |
| Should the bot refuse to start on `relativeCap == 0`? | **Yes — fail-fast at startup with an explicit error message** naming the offending market label and cap id, explaining that `0` forbids the market and that the intended "no cap" sentinel is `WAD`. See Story A2 acceptance criteria. | Decided 2026-04-07 |
